...

{
    ...
    window_=glfwCreateWindow(1244, 700, "Aselsan Comar Simulation - MuJoCo", NULL, NULL);
 ...
 */ 
    ...
    for(int leg=0; leg<4; leg++){
        ...
        // CANcommand->tau_abad_ff[leg] = legTorque(0)*ABADpole;
 ...
 * 
 * @param _cmd_ Pointer to the CANcommand struct
 */
void Simulation::SetCanCommand(CANcommand* _cmd_)
{
    cmd_=_cmd_;
}
 ...
 * 
 * @param _feedback_ Pointer to the CANData struct.
 */
void Simulation::SetCanData(CANData* _feedback_,IMUData* _bodyIMU ){
...
{
    for(int leg=0; leg<4; leg++){
        mujoco::d->qpos[(leg)*3+  kAbad_  +7]=10.75*(M_PI/180)*kSideSign_[leg];     // Add 7 to skip the first 7 dofs from body. (Position + Quaternion)
        mujoco::d->qpos[(leg)*3+  kHip_   +7]=-66.5*(M_PI/180)*kDirSign_[leg];
        mujoco::d->qpos[(leg)*3+  kKnee_  +7]=157.15*(M_PI/180)*kDirSign_[leg];
    }
